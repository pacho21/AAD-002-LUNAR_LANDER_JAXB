...
function guardarConfiguracion() {
    ...
    $.ajax({
        type: "POST",
        url: "GuardarXML", //canviar al Servlet després de comprovar que funciona.
...
function cargarXML() {
    ...
    $.ajax({
        type: "get",
        url: "CargarXML", //canviar al Servlet després de comprovar que funciona.
        dataType: "xml",
        success: function (dataXml) {
            $(dataXml).find('option').each(function (i)
            {
                var id = $(this).find('id').text();
                var nom = $(this).find('nombre').text();
                var dif = $(this).find('dificultad').text(); //$(this).attr('nombre');
                var nav = $(this).find('modeloNave').text(); //$(this).attr('edad');
                var lun = $(this).find('modeloLuna').text(); //$(this).attr('edad');
...
//devuelve la id de la opcion seleccionada para poder usarla en el xml y cargar la configuración de este.
function cargarConfiguracion() {
    var i = $("#xmlSettings option:selected").prop('id');
    crearNave(modeloNave = parseInt(confXML[i].nave), dificultad = parseInt(confXML[i].difi), modeloLuna = parseInt(confXML[i].luna));
